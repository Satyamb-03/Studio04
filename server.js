...
conn.connect((err) => {
  ...
  app.get('/user/delete/:id', (req, res) => {
    ...
    const deleteAssociationsQuery = 'DELETE FROM user_events WHERE user_id = ?';
    conn.query(deleteAssociationsQuery, [userId], (err) => {
      if (err) {
        console.error('Error deleting user events associations:', err.message);
        return res.status(500).send('Error deleting associations');
      }
